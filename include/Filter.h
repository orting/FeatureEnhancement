...
namespace feature_enhancement {
  ...
  // Volume convolve(Volume &vol1, Volume &vol2);
  ...
  void kernel(Filter3D filter, int scale, Volume &out) {
    ...
    size_t kernel_size = 3 * scale;

    size_t i =  kernel_size > half_w ? 0 : half_w - kernel_size;
    for (double x = - kernel_size; x <= kernel_size; x += 1, ++i) {
      size_t j =  kernel_size > half_h ? 0 : half_h - kernel_size;
      for (double y = -kernel_size; y <= kernel_size; y += 1, ++j) {
	size_t k =  kernel_size > half_d ? 0 : half_d - kernel_size;
	for (double z = -kernel_size; z <= kernel_size; z += 1, ++k) {
